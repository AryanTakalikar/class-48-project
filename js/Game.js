class Game {
  ...
  async start(){
   ...
   for(var j = 150;j<=600;j=j+150)
 {
  for(var k = 500;k<displayWidth*4;k = k+1000)
  {
    hurdle = createSprite(k,j,100,100);
  ...
  displayRanks(){
    ...
    clear ();
    Player.getPlayerInfo();
    background(leaderback);

    textAlign(CENTER);
    textSize(50);
    
    for(var plr in allPlayers){
      text("🏆🏆🏆Leader Board🏆🏆🏆", 0,-300)
        if(allPlayers[plr].rank === 1){
            text("🥇" + allPlayers[plr].name, -70, -170);
